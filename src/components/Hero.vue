<template>
    <section class="relative h-dvh w-full bg-black overflow-hidden">
      ...
      <div class="absolute inset-0 z-10 grid grid-cols-3 place-items-center px-4 md:px-16 text-white">
      ...
      <div class="absolute bottom-2 left-1/2 transform -translate-x-1/2 z-10">
        <h1 ref="titleRef" class="text-white uppercase custom-title">
          Sharp Glint
        </h1>
      </div>
    </section>
...
onMounted(() => {
  ...
  gsap.fromTo(
    ...
    { opacity: 0, x: 1000 },
    {
      ...
      delay: 1.5,
      onComplete: () => {
        // On force la position finale bien fixée à x=0 (important)
        gsap.set(titleRef.value, { x: 31 })

        // Animation scroll : à partir de x=0 on bouge vers la gauche
